...
def Position_Depart(M:list, nbCoup:int, nbVoit: int, etats: list, vehicules : list, sol : list):
    
    if nbCoup>=150: #Nombre maximal de coups constaté par M. X : environ 60, soit autour de 150 déplacements de 1 case
        return sol
    #if nbCoup%10==0:
        #print(nbCoup, ":")
        #print("---")
        ...
        if (y,x)!=(y1,x1): #Gestion du bord
            ...
            if M[y2+ver][x2+(not ver)]==0: #Gestion d'une collision
                ...
                if not redondant(M,etats):
                    ...
                    Mf=Position_Depart(M,nbCoup+1,nbVoit,etats,vehicules, sol)
                    """for x in Mf[0]:
                        print(x)
                    print("--------------------------")"""
                    if Mf[1]>sol[1]:
        ...
        if (y,x)!=(y1,x1): #Gestion du bord
            ...
            if M[y][x]==0: #Gestion d'une collision
                ...
                if not redondant(M,etats) :
                    ...
                    etats.append(deepcopy(M))
                    Mf=Position_Depart(M,nbCoup+1,nbVoit,etats,vehicules, sol)
                    if Mf[0][3][3:].count(0)<sol[0][3][3:].count(0):
                        sol[:]=[x for x in Mf]
                    elif Mf[0][3][3:].count(0)<sol[0][3][3:].count(0) and Mf[1]>sol[1] :
                        sol[:]=[x for x in Mf]
    ...
    if M[3][1]==1 and M[3][2]==1  : #On a une configuration intiale "suffisamment" mélangée
        #print(nbCoup)
        return [M,nbCoup]
...
def creer_Partie(M:list):
    ...
    return Position_Depart(M,0,max([max(L) for L in M]),[],T,[[[0 for x in range(6)] for y in range(6)], 0])             
...
if A is not None and A != [[0 for x in range(6)] for y in range(6)]:
